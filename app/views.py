...
def choose_payment(request, product_id):
    product = get_object_or_404(Product, id=product_id)
    # Add any logic to prepare payment options, if necessary
    return render(request, 'choose_payment.html', {'product': product})
...
def process_payment(request, product_id):
    if request.method == "POST":
        ...
        # Create an order
        order_amount = int(product.price * 100)  # Amount in paise
        order_currency = 'INR'
        order_receipt = f'order_rcpt_{product_id}'
        ...
        return redirect('choose_payment', product_id=product_id)
...
def payment(request):
    ...
    if user:
        if user.is_authenticated:
            try:
                transaction = Transaction.objects.get(customer__user=user, status='Pending')  # Get user's pending transaction
            except Transaction.DoesNotExist:
                return redirect('cart')  # Redirect to cart if no pending transaction
    else:
        # Create a temporary transaction for guest users
        transaction = Transaction.objects.filter(status='Pending').first()  # You may want to implement a better way to track guest transactions
...
def shipping(request, product_id):
    try:
        ...
        transaction = Transaction(customer=request.user.customer, product_id=product_id, status='Pending')  # Create a new transaction
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('payment', transaction_id=transaction.id)  # Redirect to payment view with transaction ID
    ...
    return render(request, 'shipping.html', {'form': form, 'transaction': transaction})
...
#     user = request.user
#     try:
#         transaction = Transaction.objects.get(customer__user=user, status='Pending')
#     except Transaction.DoesNotExist:
#         return redirect('cart')  # Redirect to cart if no pending transaction
...
#         form = PaymentForm(request.POST)
#         if form.is_valid():
#             # Here you might want to capture the payment amount and other details
#             amount = transaction.total_price * 100  # Amount in paise (Razorpay expects it in paise)
...
#             transaction.razorpay_order_id = order['id']  # Save the order ID in the transaction
#             transaction.save()
...
#                 'order_id': order['id'],
#                 'amount': amount,
#                 'key_id': "YOUR_KEY_ID"  # Your Razorpay Key ID
#             })
#     else:
#         form = PaymentForm()
